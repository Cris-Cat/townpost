...
Access Admin: Go to http://127.0.0.1:8000/admin/ and log in with your superuser credentials.
Manage Categories: Add or edit event categories (e.g., Music, Sports, Notices) and assign emojis.
Approve Events:
Navigate to Events.
Filter by Status: Pending.
Open an event, change the Status dropdown to Approved, and click Save.
...
Manage Images: You can view, reorder, or delete images directly from the Event admin page using the inline image manager.
...
Format:
The file must be a dictionary where keys are Country names and values are arrays of City names.
...
Verifies the file is a valid image.
Strips all EXIF/GPS metadata (protecting user privacy).
Resizes the image to a maximum of 1024x1024 pixels while maintaining aspect ratio.
Converts and saves the image as a high-quality JPEG to save storage space.
🤝 Contributing
